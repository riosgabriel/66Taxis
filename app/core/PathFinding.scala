...
object Astar {
  ...
  def search(world: W, s: Pos, e: Pos, heuristic: Heuristic = manhattan): List[Pos] = {
    ...
    val heap = new PriorityQueue[Node]()
    heap.enqueue(graph.nodes(s))
    // ~ (_ enqueue graph.nodes(s))
  ...
  def manhattan(pos1: Pos, pos2: Pos): Int = abs(pos2._1 - pos1._1) + abs(pos2._2 - pos1._2)
  ...
  def render(world: W, s: Pos, e: Pos, path: List[Pos] = Nil, dist: Int = 1) =
    (-dist to dist) map { row =>
      (-dist to dist) map { col =>
        (row, col) match {
          ...
          case p => world get p map (if (_) 'X' else '_') getOrElse ' '
        }
      }
